...
// 获得invoices表中的最大id值
func GetInvoicesMaxId() (max int) {
	config.Db.Table("invoices").Select("max(id)").Row().Scan(&max)
	return
}

// 获得nums个invs表中的id
func GetInvoicesId(nums int) (id []int) {
	config.Db.Debug().Table("invoices").Select("id").Limit(nums).Find(&id)
	return
}
